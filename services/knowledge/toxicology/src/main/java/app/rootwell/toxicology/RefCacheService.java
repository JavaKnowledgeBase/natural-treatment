...
public class RefCacheService {
    ...
    private static final String SYMPTOM_PREFIX = "ref:symptom:";
    ...
    public Optional<Map<String, Object>> getSymptom(String symptomId) {
        String raw = redisTemplate.opsForValue().get(SYMPTOM_PREFIX + symptomId);
        if (raw == null) {
    ...
    public List<Map<String, Object>> listSymptoms() {
        List<Map<String, Object>> symptoms = new ArrayList<>();
        for (String key : scanKeys(SYMPTOM_PREFIX + "*")) {
            String raw = redisTemplate.opsForValue().get(key);
            if (raw != null) {
                symptoms.add(parse(raw));
            }
        }
        return symptoms;
    }
