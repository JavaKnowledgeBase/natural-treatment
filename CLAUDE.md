...
- `docs/KAFKA_INTEGRATION_DESIGN.md` — **not implemented, deferred by design.** Extends the one-sentence Kafka/SQS mention in the v2 doc's async-messaging section into an actual design: where Kafka would (and deliberately would not) fit this app, using a concrete aggregate-analytics-events use case (session/safety-rule counters, no PII, no session ID) rather than the synchronous `/analyze` chain, which stays HTTP by deliberate choice since it's a genuine linear dependency with no fan-out. Written as interview-prep grounding — paired with a full standalone Kafka reference doc in the user's `Desktop/resume/` folder (`kafka-interview-reference.md`) that uses this design as its worked example
